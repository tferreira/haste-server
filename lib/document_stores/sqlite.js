...
SqliteDocumentStore.prototype = {
  ...
  tagsstats: function (callback) {
    this.safeConnect(function (err, client) {
      if (err) { return callback(false); }
      client.query("SELECT tag, count(tag) FROM tags GROUP BY tag ORDER BY count(tag) DESC", function (err, result) {
        if (err) { return callback(false); }
        if (result.length) {
          rows = []
          for (var i in result) {
  ...
  set: function (key, data, title, tags, callback, skipExpire) {
    ...
    this.safeConnect(function (err, client) {
      ...
      client.query('INSERT INTO entries (entry_key, title, value, expiration) VALUES (?,?,?,?)', [
        ...
        if (err) {
          winston.error('error inserting document to sqlite', { error: err });
          return callback(false);
        }
        // Tags
        ...
        if (tags_array.length > 0) {
          ...
          client.query(query, values, function (err, result) {
            if (err) {
              winston.error('error inserting tags to sqlite', { error: err });
              return callback(false);
            }
          });
        }
