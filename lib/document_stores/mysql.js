...
// CREATE TABLE `entries` (`id` BIGINT NOT NULL AUTO_INCREMENT , `entry_key` VARCHAR(255) NOT NULL UNIQUE, `title` VARCHAR(255) NOT NULL UNIQUE, value TEXT NOT NULL, expiration INT, PRIMARY KEY (`id`)) ENGINE=InnoDB;
...
MysqlDocumentStore.prototype = {
  ...
  // Set a given key
  set: function (key, data, title, callback, skipExpire) {
    var now = Math.floor(new Date().getTime() / 1000);
    var that = this;
    this.safeConnect(function (err, client) {
      if (err) { return callback(false); }
